...
for (const day of days) {
  const open = restaurant.openingHours[day]?.open ?? 'closed';  // ?? nulling coalescing 
...
console.log(orderSet);  // get the unique values

console.log("aaabb") // ab
...
//set don't have an indexes 
// we don't retrieve the data from the set
// No need for it
...
console.log(rest.set(2, 'Lisbon, Portugal'));   // it retuns the updated whole map{}
...
console.log(question);   // the is giving the arrays of arrays as a output , 
													// which is similar to the output of Obect.Entries() 
